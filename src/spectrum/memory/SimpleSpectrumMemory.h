...
namespace Spectrum::Memory
{
     ...
     *
     * These use a simple linear model of 16-bit addressable memory, only half of which is populated for the 16k model.
     * The MappableMemoryInterface is added to the SimpleMemory base to enable, for example, the ZX Interface 1 ROM to
     * be mapped into the address space.
     */
    class SimpleSpectrumMemory
    : public SimpleMemory<UnsignedByte>,
      public MappableMemoryInterface<UnsignedByte>
    {
    public:
        /**
         ...
         */
        explicit SimpleSpectrumMemory(Size availableSize = 0x10000);

        ~SimpleSpectrumMemory() override;
    ...
    protected:
        [[nodiscard]]
        Byte * mapAddress(Address startAddress) const override;
