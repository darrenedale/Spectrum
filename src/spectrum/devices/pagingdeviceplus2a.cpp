...
namespace
{
    ...
    constexpr UnsignedByte RomNumberLowBit = 4;
    constexpr UnsignedByte RomNumberLowMask = 1 << RomNumberLowBit;
    ...
    constexpr UnsignedByte PagingModeBit = 0;
    constexpr UnsignedByte PagingModeMask = 1 << PagingModeBit;

    // the bit and mask to apply to the written byte to determine the low bit of the ROM to page in
    constexpr UnsignedByte RomNumberHighBit = 2;
    constexpr UnsignedByte RomNumberHighMask = 1 << RomNumberHighBit;

    // the bit and mask to apply to the written byte to determine whether the disk motor should be running (1) or idle (0)
    constexpr UnsignedByte DiskMotorBit = 3;
    constexpr UnsignedByte DiskMotorMask = 1 << DiskMotorBit;

    // the bit and mask to apply to the written byte to determine the printer port strobe
    constexpr UnsignedByte PrinterStrobeBit = 4;
    constexpr UnsignedByte PrinterStrobeMask = 1 << PrinterStrobeBit;
    
    // extract the paging configuration from the byte when bit 0 indicates that special paging mode is in use
    constexpr UnsignedByte SpecialPagingConfigurationMask = 0b00000110;
    constexpr UnsignedByte SpecialPagingConfigurationShift = 1;
}
...
//    auto ramBank = static_cast<SpectrumPlus2aMemory::BankNumber>(value & RamBankMask);
    ...
    const auto rom = ((value & RomNumberLowMask) >> RomNumberLowBit)
            | (static_cast<std::uint8_t>(memory->currentRom()) & 0x02);
    memory->pageRom(rom);

    const auto screenBuffer = (value & ScreenBufferMask) ? SpectrumPlus2a::ScreenBuffer::Shadow : SpectrumPlus2a::ScreenBuffer::Normal;
