...
namespace
{
    ...
    constexpr int DisplayRefreshRate = 50;

    /** Constant for the default duration in ms of notifications generated by the MainWindow. */
    constexpr int DefaultNotificationTimeout = 5000;

    /** The width (in Spectrum pixels) of the virtual Spectrum display, including the border. */
    constexpr int DisplayWidth = DisplayDevice::Width + QImageDisplayDevice::BorderSize * 2;

    /** The height (in Spectrum pixels) of the virtual Spectrum display, including the border. */
    constexpr int DisplayHeight = DisplayDevice::Height + QImageDisplayDevice::BorderSize * 2;
...
DISABLE_WARNING_SWITCH     // we're only interested in the keys that map to the Spectrum keyboard
        ...
        switch (key) {
            case Qt::Key::Key_Backspace:
                return {Keyboard::Key::CapsShift, Keyboard::Key::Num0};
...
DISABLE_WARNING_POP
    ...
    {
        if (const auto matches = SnapshotExtensionRegularExpression.match(fileName); matches.hasMatch()) {
...
:   QMainWindow(parent),
      ...
      m_joystick(std::make_unique<Devices::KempstonJoystick>()),
      ...
      m_mouse(nullptr)
{
    ...
    connect(&m_spectrumThread, &Thread::spectrumReset, this, [this]() {
        setWindowTitle(QString::fromStdString(to_string(m_spectrum->model())));
    });
    ...
    switch (model) {
        case Model::Spectrum16k: {
            if (auto romFile = Application::romFilePath(Default16kRom); romFile.isEmpty()) {
                error = tr("The ROM file for the %1 is missing.").arg(QString::fromStdString(to_string(model)));
            } else {
        ...
        case Model::Spectrum128k: {
            if (auto romFile0 = Application::romFilePath(Default128kRom0); romFile0.isEmpty()) {
                error = tr("The %1 ROM file for the %2 is missing.").arg(tr("first"),
                                                                         QString::fromStdString(to_string(model)));
        ...
        case Model::SpectrumPlus2:
            if (auto romFile0 = Application::romFilePath(DefaultPlus2Rom0); romFile0.isEmpty()) {
                error = tr("The %1 ROM file for the %2 is missing.").arg(tr("first"), QString::fromStdString(to_string(model)));
            } else if (auto romFile1 = Application::romFilePath(DefaultPlus2Rom1); romFile1.isEmpty()) {
                error = tr("The %1 ROM file for the %2 is missing.").arg(tr("second"), QString::fromStdString(to_string(model)));
            } else {
        ...
        case Model::SpectrumPlus2a:
            if (auto romFile0 = Application::romFilePath(DefaultPlus2aRom0); romFile0.isEmpty()) {
                ...
                error = tr("The %1 ROM file for the %2 is missing.").arg(tr("third"), QString::fromStdString(to_string(model)));
            } else if (auto romFile3 = Application::romFilePath(DefaultPlus2aRom3); romFile3.isEmpty()) {
                error = tr("The %1 ROM file for the %2 is missing.").arg(tr("fourth"), QString::fromStdString(to_string(model)));
            } else {
...
bool MainWindow::loadSnapshot(const QString & fileName, QString format)
{
    ...
    if (bool canApply = m_spectrum->canApplySnapshot(*snapshot); !canApply) {
        if (snapshot->model() != m_spectrum->model()) {
            auto dlg = Dialogue(
                ...
                .arg(QString::fromStdString(to_string(m_spectrum->model())), QString::fromStdString(to_string(snapshot->model()))),
            ...
            dlg.addButton(tr("Keep %1").arg(QString::fromStdString(to_string(m_spectrum->model()))), QDialogButtonBox::ButtonRole::RejectRole);
            dlg.addButton(tr("Switch to %1").arg(QString::fromStdString(to_string(snapshot->model()))), QDialogButtonBox::ButtonRole::AcceptRole);
    ...
    setWindowTitle(QStringLiteral("%1 | %2").arg(QString::fromStdString(to_string(m_spectrum->model())), QFileInfo(fileName).fileName()));
...
void MainWindow::createDisplayMenu()
{
...

    {
        ...
        for (auto scale = 0; scale < labels.size(); ++scale) {
            auto * action = subMenu->addAction(tr(labels[scale]), [this, scale = scale + 1]() {
                // the geometry we're going to resize
                auto windowGeometry = geometry();

                // the total amount of the current width & height that isn't used by the display widget
                const auto hExtra = windowGeometry.width() - m_displayWidget.width();
                const auto vExtra = windowGeometry.height() - m_displayWidget.height();

                // resize to the desired display widget dimensions plus the width and height used by other widgets
                windowGeometry.setSize({
                    DisplayWidth * scale + hExtra,
                    DisplayHeight * scale + vExtra
                });
    ...
    for (auto colour = static_cast<ColourInt>(Colour::Black); colour <= static_cast<ColourInt>(Colour::White); ++colour) {
        auto * action = subMenu->addAction(QString::fromStdString(to_string(static_cast<Colour>(colour))), [this, colour = static_cast<Colour>(colour)]() {
            m_display.setBorder(colour, false);
        });
...
{
    // don't do anything if the resize is internally generated
    if (!event->spontaneous()) {
        return;
    }

    for (auto * action : m_displayScaleGroup.actions()) {
        action->setChecked(false);
    }
}
...
void MainWindow::useInterfaceTwoJoystickTriggered()
{
    ...
    m_joystick = std::make_unique<Devices::InterfaceTwoJoystick>();
...
void MainWindow::useCursorJoystickTriggered()
{
    ...
    m_joystick = std::make_unique<Devices::CursorJoystick>();
...
void MainWindow::useFullerJoystickTriggered()
{
    ...
    m_joystick = std::make_unique<Devices::FullerJoystick>();
...
void MainWindow::kempstonMouseToggled(bool on)
{
    ...
    if (on) {
        m_mouse = std::make_unique<Devices::KempstonMouse>();
...
bool MainWindow::loadSnapshotFromSlot(int slotIndex)
{
    ...
    setWindowTitle(QStringLiteral("%1 | Snapshot slot %2").arg(QString::fromStdString(to_string(m_spectrum->model()))).arg(slotIndex));
    return true;
}
